// 다음을 수행하시오.
const arr2 = [1, 2, 3, 4, 5];
// ex1) [2,3]을 추출
const ex1 = arr2.slice(1,3)
console.log('ex1) [2,3]을 추출 : ',ex1)

// ex2) [3]부터 모두 다 추출
const ex2 = arr2.slice(2)
console.log('ex2) [3]부터 모두 다 추출 : ',ex2)

// ex3) [2,3,4] 제거하기
const ex3=arr2.splice(1,3)
console.log('ex3) [2,3,4] 제거하기 : ',arr2,ex3)

// ex4) 복원하기
const ex4 = arr2.splice(1,0, ...ex3)
// 설명 : 1번 인덱스 위치에 0개를 제거하고 ex3의 요소들을 추가
console.log('ex4 : ', arr2,ex4)

// ex5) [3] 부터 끝까지 제거하기
const ex5=arr2.splice(2,3)
// or arr2.splice(2)
console.log("ex5 : ", arr2)

// ex6) 복원하기
ex6= arr2.splice(2,0,...ex5)
console.log("ex6 : ", arr2)



// ex7) [1,2, 'X', 'Y', 'Z', 4, 5] 만들기
// - 방법1) 3부터 모두 지우고 'X', 'Y', 'Z', 4, 5 추가
// arr2.splice(2,3,'X','Y','Z',4,5)
console.log('ex7 1 : ', arr2)

// - 방법2) 3만 지우고 'X', 'Y', 'Z' 추가
// arr2.splice(2,1,'X','Y','Z')
// or
arr2.splice(2,1,...'XYZ')
console.log('ex7 2 : ', arr2)

// ex8) 위 7번 문제를 splice를 사용하지 말고 작성하시오.
const ex8 = [...arr2.slice(0,2),...'XYZ',...arr2.slice(-2)]
console.log('ex8 : ', ex8)